...
#
paths = {}
paths["hdf5"] = os.getcwd() + "/forecast_files/"
paths["netCDF"] = os.getcwd() + "/forecast_files/"


# In[3]:


def load_forecasts(paths):
    today_date = (datetime.datetime.now()).strftime("%Y%m%dT000000Z")
    filename = paths["hdf5"] + "AICE_forecasts_" + today_date + ".h5"
...
def write_netCDF_forecasts(Dataset):
    lead_time_max = 10
    today_date = (datetime.datetime.now()).strftime("%Y%m%dT000000Z")
    time_coverage_first = (datetime.datetime.now()).strftime("%Y-%m-%dT000000Z")
    time_coverage_last = (datetime.datetime.now() + datetime.timedelta(days = lead_time_max)).strftime("%Y-%m-%dT000000Z")
    #
    path_output = paths["netCDF"] 
    if os.path.exists(path_output) == False:
        os.system("mkdir -p " + path_output)    
    output_filename = path_output + "AICE_forecasts_" + today_date + ".nc"
...
Dataset = load_forecasts(paths)
write_netCDF_forecasts(Dataset)
