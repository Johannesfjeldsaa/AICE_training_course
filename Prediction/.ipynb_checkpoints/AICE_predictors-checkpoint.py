...
paths["AMSR2"] = "/lustre/storeB/project/fou/fd/operational/nrt_l4/"
paths["ECMWF"] = "/lustre/storeB/project/metproduction/products/ecmwf/nc/"
...
def extract_domain_and_LSM(paths = paths):
    xmin = 909
    xmax = 1453
    ymin = 1075
    ymax = 1555
...
class create_AMSR2_predictor():
    def __init__(self, Domain_data, paths, crs):
...
def create_ECMWF_predictors(Domain_data, paths = paths):
    lead_time_max = 10
...
def write_hdf5_predictors(Datasets):
    today_date = (datetime.datetime.now()).strftime("%Y%m%dT000000Z")
    timestamps = []
    #
    for lt in range(0, 10):
        timestamps.append((datetime.datetime.now() + datetime.timedelta(days = lt)).timestamp())
    #
    path_output = paths["output"] 
    if os.path.exists(path_output) == False:
        os.system("mkdir -p " + path_output)    
    output_filename = path_output + "AICE_predictors_" + today_date + ".h5"
...
# # Function write_netCDF_predictors

# In[299]:


def write_netCDF_predictors(Datasets):
    lead_time_max = 10
    today_date = (datetime.datetime.now()).strftime("%Y%m%dT000000Z")
    timestamps = []
    ...
    if os.path.exists(path_output) == False:
        os.system("mkdir -p " + path_output)    
    output_filename = path_output + "AICE_predictors_" + today_date + ".nc"
    if os.path.isfile(output_filename):
        os.system("rm " + output_filename)
    ...
    #
    crs = output_netcdf.createVariable("crs", "int")
    ...
    SICobs_AMSR2 = output_netcdf.createVariable("SICobs_AMSR2", "d", ("y", "x"))
    ECMWF_T2M = output_netcdf.createVariable("ECMWF_T2M", "d", ("time", "y", "x"))
    ECMWF_x_wind = output_netcdf.createVariable("ECMWF_x_wind", "d", ("time", "y", "x"))
    ECMWF_y_wind = output_netcdf.createVariable("ECMWF_y_wind", "d", ("time", "y", "x"))
    #
    ...
    crs.proj4_string = "+ellps=WGS84 +lat_0=90 +lon_0=0 +no_defs=None +proj=laea +type=crs +units=m +x_0=0 +y_0=0"
    ...
    LSM.units = "1: ocean, 0: land"
    SICobs_AMSR2.standard_name = "Sea ice concentration from AMSR2 during the day preceding the forecast start date"
    SICobs_AMSR2.units = "%"
    ECMWF_T2M.standard_name = "ECMWF 2 metre temperature"
    ECMWF_T2M.units = "K"
    ECMWF_x_wind.standard_name = "Mean ECMWF wind in the x direction since the forecast start date"
    ECMWF_x_wind.units = "m/s"
    ECMWF_y_wind.standard_name = "Mean ECMWF wind in the y direction since the forecast start date"
    ECMWF_y_wind.units = "m/s"
    #
    time[:] = timestamps
    ...
    LSM[:,:] = np.copy(Datasets["Domain_data"]["LSM"])
    SICobs_AMSR2[:,:] = np.copy(Datasets["SICobs_AMSR2_SIC"])
    ECMWF_T2M[:,:,:] = np.copy(Datasets["ECMWF"]["T2M_cum"])
    ECMWF_x_wind[:,:,:] = np.copy(Datasets["ECMWF"]["wind_x_cum"])
    ECMWF_y_wind[:,:,:] = np.copy(Datasets["ECMWF"]["wind_y_cum"])
    #
    output_netcdf.close()
...
#
Datasets = {}
Datasets["Domain_data"] = extract_domain_and_LSM()
t1 = time.time()
print("............................................................................................")
print("AICE predictors")
print("Domain_data", t1 - t0)
#
Datasets["SICobs_AMSR2_SIC"] = create_AMSR2_predictor(Datasets["Domain_data"], paths, crs).produce_AMSR2_SIC()
t2 = time.time()
print("AMSR2 SIC predictor", t2 - t1)
#
Datasets["ECMWF"] = create_ECMWF_predictors(Datasets["Domain_data"])
#  
write_hdf5_predictors(Datasets)
#write_netCDF_predictors(Datasets)
#
t3 = time.time()
print("ECMWF predictors", t3 - t2)
print("AICE predictors computing time", t3 - t0)
